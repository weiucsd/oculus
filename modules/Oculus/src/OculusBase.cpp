...
void OculusBase::Run(OculusScene &oculusscene)
{
  ...
  projection_matrici[ovrEye_Left] = ovrMatrix4f_Projection(eye_render_desc_[ovrEye_Left].Fov, 0.3f, 100.0f, true);
  projection_matrici[ovrEye_Right] = ovrMatrix4f_Projection(eye_render_desc_[ovrEye_Right].Fov, 0.3f, 100.0f, true);

  // IPD offset values will not change at runtime, we can set them here
  ovrVector3f eye_offsets[2];
  eye_offsets[ovrEye_Left] = eye_render_desc_[ovrEye_Left].HmdToEyeViewOffset;
  eye_offsets[ovrEye_Right] = eye_render_desc_[ovrEye_Right].HmdToEyeViewOffset;
  ...
  {
    // Begin the frame
    ovrHmd_BeginFrame(hmd_, frame_index);
    ...
    ovrHmd_GetEyePoses(hmd_, frame_index, eye_offsets, eye_poses, NULL);
    ...
    for (int eye_index=0; eye_index<ovrEye_Count; eye_index++)
    {
      ...
      glLoadIdentity();
      glMultMatrixf(&(projection_matrici[eye_type].Transposed().M[0][0]));
      ...
      OVR::Quatf oculus_orientation = OVR::Quatf(eye_poses[eye_type].Orientation);
      ...
      glTranslatef(-eye_poses[eye_type].Position.x, -eye_poses[eye_type].Position.y, -eye_poses[eye_type].Position.z);
    ...
    ovrHmd_EndFrame(hmd_, eye_poses, eye_textures_);
